...
var workspace = "12345";
var skippedTeams = [1234567, 2224555111];
...
function TestFunc()
{
	var orgTeams = GetTeams(workspace);
	Logger.log("Teams:\n" + orgTeams + "\n\n");
	var orgProjects = GetProjectsFromTeams(orgTeams);
	Logger.log("Projects:\n" + orgProjects + "\n\n");
	WriteProjectDetails(orgProjects);
}
...
function GetTeams(workspace)
{
  ...
  do
  {
    ...
    jResponse = JSON.parse(response);
		...
		// If there are more than 100 teams
...
function GetProjectsFromTeams(arrTeams)
{
  var projects = [];
  var offset = "";
  var options = { "method" : "get"};
  ...
  options.headers["Asana-Fast-Api"] = "true";
	...
	{
		if (skippedTeams.indexOf(arrTeams[i]) > -1)
			continue;
...
function WriteProjectDetails(projects)
{
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName('Live source data');

  var rangesToClear = ['A3:K5000'];
