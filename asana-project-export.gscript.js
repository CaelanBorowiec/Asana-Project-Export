...
function AuthData()
{
	return {"Authorization": "Basic " + Utilities.base64Encode("YOUR-API-KEY" + ":" + "")};
}
...
function GetProjects()
{
  ...
  var response = UrlFetchApp.fetch("https://app.asana.com/api/1.0/projects?limit=100&workspace=YOURWORKSPACE&archived=false", options);
  ...
  for(var i in data)
  {
    var options = { "method" : "get"};
    options.headers = AuthData();
    //Logger.log("https://app.asana.com/api/1.0/projects/"+data[i].id);
    var projectResponse = UrlFetchApp.fetch("https://app.asana.com/api/1.0/projects/"+data[i].id, options);
    //Logger.log("Response:  " + projectResponse.getResponseCode() + "\n\nContent:\n" + projectResponse.getContentText());
